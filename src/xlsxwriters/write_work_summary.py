...
from utils.xlsx_formula_utils import work_billing_summary_formula, work_cost_summary_formula, wrap_catch_error
import pandas as pd
...
def write_work_summary(workbook, worksheet, billing_sched_df, activities_df):
    ...
    worksheet.write(0,9, 'August', heading_format2(workbook))
    worksheet.write(0,10, 'September', heading_format2(workbook))
    worksheet.write(0,11, 'October', heading_format2(workbook))
    worksheet.write(0,12, 'November', heading_format2(workbook))
    worksheet.write(0,13, 'December', heading_format2(workbook))
        ...
        for index_y,category in enumerate(sub_dic[subcontractor]):
            ...
            for index_z,area in enumerate(sub_dic[subcontractor][category]):
                ...
                for col_index in range(10):
                    worksheet.write(row,4+col_index, wrap_catch_error(work_billing_summary_formula(row, col_index, sub_row, index_y, index_z)), percent_format(workbook, '#ffffff'))
                    
                worksheet.write(row+1, 3, 'Costs', string_format(workbook))
                for col_index in range(10):
                    worksheet.write(row+1,4+col_index, wrap_catch_error(work_cost_summary_formula(row, col_index, sub_row, index_y, index_z, subcontractor)), percent_format(workbook, '#ffffff'))
    ...
    worksheet.conditional_format(f"E4:N{row}", {"type": "cell", "criteria": "=", "value": '\"N/A\"', "format": summary_text_light(workbook)})
    worksheet.conditional_format(f"E4:N{row}", {"type": "cell", "criteria": "=", "value": 0, "format": summary_text_light(workbook)})
    worksheet.conditional_format(f"E4:N{row}", {"type": "cell", "criteria": ">", "value": 0, "format": summary_text_red(workbook)})
    # worksheet.conditional_format(f"E4:N{row}", {"type": "text", "criteria": "containing", "value": '"N/A"', "format": summary_text_light_with_white_bg(workbook)})
